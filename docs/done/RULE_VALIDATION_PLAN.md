...
> `src/RulesEngine.Configuration/Validation/` and `Loading/`, `CliRepositoryContext.ValidateRules()`,
> the new `check-rules` command (`src/RulesEngine.Cli/Commands/CheckRulesCommand.cs`), the shared
> `RuleValidationReportWriter`, and the mandatory pre-flight gate in `ValidateCommand`. Covered by
> new tests in `RuleFileLoaderTests` and `RulesEngine.Cli.Tests` (`CheckRulesCommandTests`,
...
(`src/RulesEngine.Configuration/Loading/RuleFileLoader.cs`), plus a duplicate-rule-ID check across a
...
- **Uncaught in the CLI**: no command catches these exceptions. `rules-engine validate` against a
...
- **No standalone entry point**: there is no way to check a folder of rule YAML in isolation — you
  ...
  MSBuild, etc.) against rules wired into `.rulesengine/config.yml`.
...
### 1. Non-throwing aggregate validation in `RulesEngine.Configuration`

Add to `RulesEngine.Configuration/Validation/`:
...
Refactor `RuleFileLoader` (`src/RulesEngine.Configuration/Loading/RuleFileLoader.cs`) around a new
non-throwing core:
...
- `LoadFromFile` becomes a thin wrapper: call `TryLoadFromFile`, throw the first error (as today) if
  ...
  `tests/RulesEngine.Configuration.Tests/RuleFileLoaderTests.cs` unchanged (they assert
...
`src/RulesEngine.Cli/Support/CliRepositoryContext.cs`, calling
...
`src/RulesEngine.Cli/Commands/CheckRulesCommand.cs`, registered in `Program.cs` alongside the other
subcommands. Options, reusing `CommonOptions` (`src/RulesEngine.Cli/Support/CommonOptions.cs`) exactly
...
- A directly-supplied folder → reuse `--rules-source` (already means "ad-hoc rules location, local
  directory or git URL, bypassing `.rulesengine/config.yml`") rather than inventing a second
...
```
rules-engine check-rules                              # whatever this repo is configured to use
rules-engine check-rules --rules-source ./some/rules   # ad-hoc folder, no repo/config needed
```
...
In `ValidateCommand.Build()`'s `SetAction` (`src/RulesEngine.Cli/Commands/ValidateCommand.cs`),
...
small `RuleValidationReportWriter` in `RulesEngine.Cli/Support/`, rather than duplicating it.

### 5. Tests

- `tests/RulesEngine.Configuration.Tests/RuleFileLoaderTests.cs`: keep all existing tests passing
...
- `tests/RulesEngine.Cli.Tests/`: add `CheckRulesCommandTests` (valid folder → exit 0; folder with a
...
- Manually: `dotnet run --project src/RulesEngine.Cli -- check-rules` against this repo's own
  ...
  in one run.
- Manually: `dotnet run --project src/RulesEngine.Cli -- validate` with one rule file temporarily
