...
public class MustAllMatchAssertionTests
{
    ...
    private static MustAllMatchAssertion Assertion(IEnumerable<object> candidates, params IAssertion[] assertions) =>
        new(Template, _ => new StubSelector(candidates), assertions);

    [Fact]
    public void Evaluate_Passes_WhenEveryMatchSatisfiesEveryNestedAssertion()
    {
        var alwaysPasses = new StubAssertion(_ => true, "never happens");

        var outcome = Assertion([TestModels.Type("A"), TestModels.Type("B")], alwaysPasses)
    ...
    {
        var alwaysFails = new StubAssertion(_ => false, "should never be evaluated");

        var outcome = Assertion([], alwaysFails).Evaluate(TestModels.Type("Outer"), EmptyModel);

        Assert.True(outcome.Passed);
    }

    [Fact]
    public void Evaluate_Fails_WhenAnyMatchFailsAnyNestedAssertion()
    {
        ...
        Assert.Contains("B", outcome.Message);
        Assert.Contains("must be named A", outcome.Message);
    }
}
