...
public class RulesEngineConfigLoaderTests : IDisposable
{
    private readonly string _repoRoot = Directory.CreateTempSubdirectory("rulesengine-config-").FullName;

    [Fact]
    public void LoadOrDefault_ParsesConfigFile_WhenPresent()
    {
        var rulesEngineDir = Directory.CreateDirectory(Path.Combine(_repoRoot, ".rulesengine"));
        ...
        var config = RulesEngineConfigLoader.LoadOrDefault(_repoRoot);
    ...
    public void LoadOrDefault_UsesExplicitConfigPath_WhenProvided()
    {
        ...
        var config = RulesEngineConfigLoader.LoadOrDefault(_repoRoot, explicitConfigPath);
    ...
    public void LoadOrDefault_Throws_WhenExplicitConfigPathDoesNotExist()
    {
        ...
        Assert.Throws<FileNotFoundException>(() => RulesEngineConfigLoader.LoadOrDefault(_repoRoot, missingPath));
    }
