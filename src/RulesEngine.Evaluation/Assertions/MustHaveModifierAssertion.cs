using RulesEngine.Analysis.AnalysisModel;
using RulesEngine.RuleModel.Assertions;

namespace RulesEngine.Evaluation.Assertions;

public sealed class MustHaveModifierAssertion(string modifier) : IAssertion
{
    public string Kind => "must_have_modifier";

    public AssertionOutcome Evaluate(object candidate, RepositoryModel model)
    {
        var matches = ModifierMatcher.Matches(candidate, modifier);
        return matches switch
        {
            null => AssertionOutcome.Failure($"Modifier '{modifier}' is not applicable to this candidate."),
            true => AssertionOutcome.Success(),
            false => AssertionOutcome.Failure($"Candidate must have modifier '{modifier}'.")
        };
    }
}
